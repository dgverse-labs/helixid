...
describe('HelixClient Branch Coverage', () => {
    ...
    beforeEach(() => {
        ...
        client = new HelixClient(mockHttp, 'http://localhost');
    });
    it('constructor handles string baseUrl', () => {
        const c2 = new HelixClient('http://localhost');
        expect(c2.http).toBeInstanceOf(HttpAdapter);
    });
    ...
    describe('adapter checks', () => {
        it('throws for removeServiceEndpoint if DELETE missing', async () => {
            const c2 = new HelixClient({ post: vi.fn() }, 'http://localhost');
    ...
    describe('checkVCStatus branches', () => {
        it('returns expired if date in past', async () => {
            const res = await client.checkVCStatus({ validUntil: new Date(Date.now() - 1000).toISOString() });
...
//# sourceMappingURL=HelixClient.test.js.map